...
export const fractalEngine = wgslFn(`
  fn fractalRender(
    ...
    if (isHit) {
      ...
      let dC = getFractalDistance(pG, uType, uParams, uSettleTime, zoomLOD, uSlicerEnabled, uSlicerOffset, uSlicerAxis);
      
      let pX = (hitPoint + vec3<f32>(epsilon, 0.0, 0.0)) / uZoom + vec3<f32>(uOff, 0.0);
      let dX = getFractalDistance(pX, uType, uParams, uSettleTime, zoomLOD, uSlicerEnabled, uSlicerOffset, uSlicerAxis);
      
      let pY = (hitPoint + vec3<f32>(0.0, epsilon, 0.0)) / uZoom + vec3<f32>(uOff, 0.0);
      let dY = getFractalDistance(pY, uType, uParams, uSettleTime, zoomLOD, uSlicerEnabled, uSlicerOffset, uSlicerAxis);
      
      let pZ = (hitPoint + vec3<f32>(0.0, 0.0, epsilon)) / uZoom + vec3<f32>(uOff, 0.0);
      let dZ = getFractalDistance(pZ, uType, uParams, uSettleTime, zoomLOD, uSlicerEnabled, uSlicerOffset, uSlicerAxis);
  ...
  fn getFractalDistance(
    ...
    if (fractalType == 0) {
      let iter = i32(mix(4.0, f32(maxIterations) + zoomLOD * 0.5, settleTime));
      dist = getMandelbulbDistance(point, params.y, iter);
    } else if (fractalType == 1) {
      let iter = i32(mix(4.0, f32(maxIterations) + zoomLOD * 0.3, settleTime));
      dist = getMengerSpongeDistance(point, params.y, iter);
    } else if (fractalType == 2) {
      let iter = i32(mix(48.0, f32(maxIterations) + zoomLOD * 0.8, settleTime));
      dist = getJuliaDistance(point, vec3<f32>(params.z, params.w, 0.1), iter);
    } else if (fractalType == 3) {
      let iter = i32(mix(14.0, f32(maxIterations) + zoomLOD, settleTime));
      dist = getSierpinskiDistance(point, params.y, iter);
    } else if (fractalType == 4) {
      let iter = i32(mix(8.0, f32(maxIterations) + zoomLOD * 0.4, settleTime));
